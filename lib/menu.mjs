...
export function buildMainMenu(stops, state, defaultThresholdMinutes) {
  ...
  inlineKeyboard.push([{ text: "🏖 休假模式", callback_data: "m:vac" }]);
...
export function buildVacationMenu(vacationUntil, todayKey) {
  const active = vacationUntil && vacationUntil >= todayKey;
  const rows = [];

  for (let index = 0; index < VACATION_CHOICES.length; index += 2) {
    rows.push(
      VACATION_CHOICES.slice(index, index + 2).map((choice) => ({
        text: choice.label,
        callback_data: `m:vacset:${choice.days}`,
      })),
    );
  }
  if (active) {
    rows.push([{ text: "🔔 结束休假，恢复提醒", callback_data: "m:vacset:0" }]);
  }
  rows.push([BACK_BUTTON]);
  ...
  return {
    text: [
      "🏖 休假模式",
      "",
      active ? `当前：休假到 ${vacationUntil}（含当天）` : "当前：未在休假",
      "",
      "休假期间不发主动提醒，",
      "但手动查询「状态」照常可用。",
    ].join("\n"),
