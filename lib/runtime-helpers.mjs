...
export const DEFAULT_ACTIVE_WEEKDAYS = ["Mon", "Tue", "Wed", "Thu", "Fri"];

export function normalizeActiveWeekdays(value) {
  if (!Array.isArray(value)) {
    return [...DEFAULT_ACTIVE_WEEKDAYS];
  }
  const selected = WEEKDAY_ORDER.filter((day) => value.includes(day));
  return selected.length > 0 ? selected : [...DEFAULT_ACTIVE_WEEKDAYS];
}

export function describeActiveWeekdays(activeWeekdays) {
  const days = normalizeActiveWeekdays(activeWeekdays);
  if (days.length === 7) {
    return "每天";
  }
  if (days.join(",") === DEFAULT_ACTIVE_WEEKDAYS.join(",")) {
    return "周一至周五";
  }
  return `周${days.map((day) => WEEKDAY_LABELS[day]).join("、")}`;
}
...
export function windowPeriodKey(window) {
  return hhmmToMinute(window.start) < 12 * 60 ? "早" : "晚";
}

export function stopsForPeriod(stops, periodKey) {
  return (stops || []).filter(
    (stop) =>
      !Array.isArray(stop.periods) || stop.periods.length === 0 || stop.periods.includes(periodKey),
  );
}

export function pickDepartureCandidate(arrivals, walkMinutes, bandMinutes = 2) {
  ...
  for (const arrival of arrivals || []) {
    ...
    const etaMinutes = arrival.duration_ms / 60000;
    if (etaMinutes < walkMinutes) {
      continue;
    }
    return etaMinutes <= walkMinutes + bandMinutes ? arrival : null;
  }

  return null;
}

export const GHOST_DRIFT_SECONDS = 240;
export const GHOST_MIN_REMAINING_SECONDS = 90;

// A bus that actually arrives also vanishes from the feed, so only treat a
// disappearance as a ghost when the bus still had time left on the clock.
export function detectEtaAnomaly(previous, currentDurationMs, nowMs, options = {}) {
  const driftSeconds = options.driftSeconds ?? GHOST_DRIFT_SECONDS;
  const minRemaining = options.minRemainingSeconds ?? GHOST_MIN_REMAINING_SECONDS;

  if (!previous || !Number.isFinite(previous.durationMs) || !Number.isFinite(previous.at)) {
    return null;
  }

  const elapsedSeconds = (nowMs - previous.at) / 1000;
  if (elapsedSeconds < 0) {
    return null;
  }

  const expectedSeconds = previous.durationMs / 1000 - elapsedSeconds;
  if (expectedSeconds <= minRemaining) {
    return null;
  }

  const actualSeconds = Number.isFinite(currentDurationMs)
    ? currentDurationMs / 1000
    : Number.POSITIVE_INFINITY;

  if (actualSeconds - expectedSeconds < driftSeconds) {
    return null;
  }

  return {
    kind: Number.isFinite(currentDurationMs) ? "delayed" : "vanished",
    expectedMinutes: Math.max(0, Math.round(expectedSeconds / 60)),
    actualMinutes: Number.isFinite(actualSeconds) ? Math.round(actualSeconds / 60) : null,
  };
}

export function parseDateKeySet(value) {
  const keys = new Set();
...
export function buildTelegramButtons(stops, isMuted) {
  ...
  inlineKeyboard.push([{ text: "⚙️ 设置", callback_data: "m:main" }]);
