...
export async function pruneHistory(filePath, keepDays = 180) {
  ...
  try {
    ...
    const { stdout } = await execFileAsync("sqlite3", [
      dbPath,
      [
        `DELETE FROM alert_history WHERE sent_at < datetime('now', '-${Number(keepDays)} days');`,
        "SELECT changes();",
        "VACUUM;",
      ].join("\n"),
    ]);
...
async function persistStateToDb(dbPath, state) {
  ...
  if (normalized.vacationUntil) {
    commands.push(`INSERT OR REPLACE INTO settings (key, value) VALUES ('vacationUntil', ${sqlValue(normalized.vacationUntil)});`);
  } else {
    commands.push("DELETE FROM settings WHERE key = 'vacationUntil';");
  }
